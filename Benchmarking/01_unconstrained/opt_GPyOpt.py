...
def GPyOpt_BO(
    ...
    domain = [{'name': 'var_' + str(i+1), 'type': 'continuous', 'domain': bounds[i]} for i in range(x_dim)]

    # Initialize Bayesian Optimization
    bo = GPyOpt.methods.BayesianOptimization(f=f.fun_test, domain=domain, initial_design_numdata=n_rs, initial_design_type='random', exact_feval=True)
