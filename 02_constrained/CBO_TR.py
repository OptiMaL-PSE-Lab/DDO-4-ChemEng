...
class ITR_GP_RTO:
    ...
    def Adjust_TR(self, Delta0, xk, xnew, GP_obj, i_rto):
        ...
        if plant_iplus<plant_i: # this is what we want, since we want to minimize the objective system
            ...
            Delta0                  = Delta0*gamma_red
            self.backtrack_l[i_rto] = True
            print('plant_iplus<plant_i -- backtracking <--- shouldnt this be another print statement?')
    ...
    def Ball_sampling(self, ndim):
        ...
        '''
        print('ITR_GP_RTO: Ball_sampling: providing random positions d_init in radius in a circle with radiuis of the trust region in each input dimension')
    ...
    def RTO_routine(self):
        ...
        for i_rto in range(n_iter):
            ...
            for j in range(multi_opt):
                #print('ITR_GP_RTO: RTO_routine: rto -- iterations: iteration: ' + str(i_rto+1) + ', multi_start: ' + str(j+1) + ': Ball_sampling')
